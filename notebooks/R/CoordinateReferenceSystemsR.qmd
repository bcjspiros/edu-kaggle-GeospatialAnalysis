...
start_gdf <- birds %>% 
    ...
    summarize(
        .by = tag_local_identifier,
        # convert the MULTIPOINT obj from st_combine into a list of POINTs,
        # then we grab the first one for summarize to set the geometry col to 
...
end_gdf <- birds %>% 
    ...
    summarize(
        .by = tag_local_identifier,
        # convert the MULTIPOINT obj from st_combine into a list of POINTs,
        # then we grab the last one for summarize to set the geometry col to 
        geometry = st_combine(geometry) %>%
            st_cast('POINT') %>%
            .[length(.)]
    ) %>% 
