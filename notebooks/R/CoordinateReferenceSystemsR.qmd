...
world <- world %>% rename(continent = contnnt)
```
```{r}
ne_world <- read_sf(here("data/ne_110m_admin_0_countries/ne_110m_admin_0_countries.shp"))
```
...
for (col in col_names) {
  if (nchar(col) > 11) {
    ...
    new_col <- c(new_col, str_trunc(col, 11, ellipsis = ""))
  }
}
```
...
```{r}
st_write(World, here("data/tmap_World/tmap_World.geojson"))
```
